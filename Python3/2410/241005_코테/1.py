...
#
# 예제:
# 1. R = [0,2,1,1,0,4,1]이라 할 때, 상기 함수의 리턴 값은 8입니다. 깊이 [2,1,1]의 각 개별 포트홀이 하나의 포트홀 그룹으로 묶인다고 했을 때,
# 이 포트홀 그룹은 3개 세그먼트로 구성되며 가장 깊은 깊이값은 2가 되므로 포트홀 인디케이터 값은 3 * 2 = 6가 됩니다.
# 깊이 [4,1]의 각 개별 포트홀이 하나의 포트홀 그룹으로 묶인다고 했을 때, 이 포트홀 그룹은 연속된 2개의 포트홀로 구성되며
# 가장 깊은 깊이값은 4이므로 포트홀 인디케이터 값은 2*4 = 8이 됩니다.
#
...
def solution(r):
    ...
    answer2=map(lambda x:len(x)*max(x),answer)
    return max(answer2)
